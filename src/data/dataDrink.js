const drinkData = {
    "Cafetería": [
        {
            id: 1,
            name: "Pocillo",
            price: 2.500
        },
        {
            id: 2,
            name: "Jarrito",
            price: 2.700
        },
        {
            id: 3,
            name: "Café Doble",
            price: 2.900
        },
        {
            id: 4,
            name: "Té",
            price: 2.600
        },
        {
            id: 5,
            name: "Té con Leche",
            price: 2.950
        },
        {
            id: 6,
            name: "Chocolatada",
            price: 2.600
        },
        {
            id: 7,
            name: "Submarino",
            price: 2.950
        },
        {
            id: 8,
            name: "Capuchino",
            price: 2.950
        }
    ],
    "Licuados y Exprimidos": [
        {
            id: 9,
            name: "Banana",
            price: 3.500
        },
        {
            id: 10,
            name: "Durazno",
            price: 3.600
        },
        {
            id: 11,
            name: "Naranja",
            price: 3.500
        },
        {
            id: 12,
            name: "Limónada",
            price: 3.300
        },
        {
            id: 13,
            name: "Multifruta",
            price: 3.800
        },
        {
            id: 14,
            name: "Jarra de pomelada con lima y Menta",
            price: 4.500
        },
        {
            id: 15,
            name: "Jarra de limonada con menta y Jengibre",
            price: 4.500
        }
    ],
    "Opciones Detox": [
        {
            id: 16,
            name: "Green",
            description: "Equilibrio perfecto: Espinaca, manzana verde, ananá, pera y limón",
            price: 3.900
        },
        {
            id: 17,
            name: "Orange",
            description: "Nutrición y vitalidad: Zanahoria, naranja roja, jengibre y limón",
            price: 3.900
        },
        {
            id: 18,
            name: "Fresh",
            description: "Sabor y frescura: Limón, pepino, menta y jengibre",
            price: 3.900
        }
    ],
    "Bebidas": [
        {
            id: 19,
            name: "Agua con y sin Gas - Saborizadas",
            price: 2.200
        },
        {
            id: 20,
            name: "Gatorade",
            price: 2.300
        },
        {
            id: 21,
            name: "Pepsi Común y Black | 7up - 7Up Free | Paso de los Toros",
            price: 2.200
        },
        {
            id: 22,
            name: "Stella Artois 1lt",
            price: 6.800
        },
        {
            id: 23,
            name: "Andes 1lt",
            price: 5.600
        },
        {
            id: 40,
            name: "Lata Stella sin Alcohol 473ml",
            price: 2.550
        },
        {
            id: 41,
            name: "Lata Stella artois 473ml",
            price: 2.550
        },
        {
            id: 42,
            name: "Lata Andes rubia 473ml",
            price: 2.250
        }
    ]
};

export default drinkData;
